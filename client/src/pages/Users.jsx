...
export default function Users({ refreshKey, onRefresh }) {
  ...
  const editUser = (item) => {
    ...
    setForm({ nama: item.nama, email: item.email, password: '', role: item.role, bidang_id: item.bidang_id || '', jabatan: item.jabatan || '', sub_bidang: item.sub_bidang || '', no_telepon: item.no_telepon || '', can_patroli: Boolean(item.can_patroli) });
    setShowForm(true);
    setError('');
    setSuccess('');
  };
  ...
  const togglePatrolAccess = async (item) => {
    try {
      await api.updateUser(item.id, {
        ...
        can_patroli: !Boolean(item.can_patroli)
      });
      setSuccess(`Hak patroli ${item.nama} berhasil ${item.can_patroli ? 'dinonaktifkan' : 'diaktifkan'}.`);
      onRefresh();
    } catch (err) {
      setError(err.message || 'Gagal mengubah hak patroli.');
    }
  };
