...
export default function PatrolDashboard({ refreshKey, onRefresh }) {
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(map.values()).map((group) => {
      const entries = [...group.entries].sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));
    ...
    }).sort((a, b) => new Date(b.latest?.tanggal || 0) - new Date(a.latest?.tanggal || 0));
  }, [patrols]);
          ...
          <tbody className="divide-y divide-slate-100">
            {groups.map((group) => (
              <tr key={group.ruangan} onClick={() => setActiveGroup(group)} className="cursor-pointer transition hover:bg-emerald-50/60">
                ...
                <td className="px-4 py-3 text-slate-500">{group.latest ? new Date(`${group.latest.tanggal}T00:00:00`).toLocaleDateString('id-ID') : '-'}</td>
          ...
          {activeGroup.entries.map((patrol) => (
            <article key={patrol.id} onClick={() => setSelected(patrol)} className="cursor-pointer rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-white hover:shadow-md">
              <div className="flex items-start justify-between gap-3">
                <p className="flex items-center gap-1 text-[11px] font-bold text-slate-600"><CalendarDays className="h-3 w-3" />{new Date(`${patrol.tanggal}T00:00:00`).toLocaleDateString('id-ID')}</p>
