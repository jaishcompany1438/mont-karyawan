...
const colorClass = { MERAH: 'bg-rose-500', ORANYE: 'bg-orange-400', HIJAU: 'bg-blue-500' };
...
export default function PatrolDashboard({ refreshKey, onRefresh }) {
  ...
  return <div className="space-y-6">
    <div><p className="text-xs font-bold uppercase tracking-wider text-blue-700">Kontrol fasilitas</p><h2 className="mt-1 text-2xl font-extrabold text-slate-900">Patroli Rumah Tangga</h2><p className="mt-2 text-sm text-slate-500">Pantau kebersihan, kerapihan, sarana prasarana, dan ketertiban area.</p></div>
    ...
    <section>
      <div className="mb-3 flex items-center justify-between"><h3 className="flex items-center gap-2 text-sm font-extrabold text-slate-900"><ClipboardCheck className="h-4 w-4 text-blue-600" />{findingsOnly ? 'Patroli dengan Temuan' : 'Daftar Patroli'}</h3><span className="text-xs text-slate-500">{groups.length} area · {patrols.length} hasil</span></div>
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100">
            {groups.map((group) => (
              <tr key={group.ruangan} onClick={() => setActiveGroup(group)} className="cursor-pointer transition hover:bg-blue-50/60">
                <td className="px-4 py-3 font-extrabold text-slate-900"><span className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-blue-600" />{group.ruangan}</span></td>
    ...
    {activeGroup && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/60 p-4" onClick={() => setActiveGroup(null)}>
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between">
          <div><p className="text-xs font-bold uppercase text-blue-700">Riwayat Area</p><h3 className="mt-1 text-2xl font-extrabold text-slate-900">{activeGroup.ruangan}</h3><p className="mt-1 text-xs text-slate-500">{activeGroup.total} catatan patroli · {activeGroup.findings} temuan</p></div>
        ...
        <div className="mt-4 space-y-3">
          {activeGroup.entries.map((patrol) => (
            <article key={patrol.id} onClick={() => setSelected(patrol)} className="cursor-pointer rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-white hover:shadow-md">
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/60 p-4" onClick={() => setSelected(null)}><div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase text-blue-700">Detail Patroli</p><h3 className="mt-1 text-2xl font-extrabold text-slate-900">{selected.ruangan}</h3></div><button onClick={() => setSelected(null)} className="text-2xl text-slate-400">&times;</button></div><div className="mt-4 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><span><CalendarDays className="mr-1 inline h-3.5 w-3.5" />{formatPatrolDate(selected.tanggal)}</span><span><User className="mr-1 inline h-3.5 w-3.5" />{selected.petugas_nama}</span></div><div className="mt-5"><Indicators patrol={selected} /></div>{selected.foto_bukti && <a href={selected.foto_bukti} target="_blank" rel="noreferrer" className="mt-5 block overflow-hidden rounded-2xl border border-slate-200"><img src={selected.foto_bukti} alt={`Bukti ${selected.ruangan}`} className="max-h-72 w-full object-cover transition hover:scale-105" /><span className="flex items-center gap-2 p-3 text-xs font-bold text-blue-700"><Camera className="h-4 w-4" />Buka foto bukti</span></a>}<div className="mt-5 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Catatan Temuan</p><p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{selected.catatan_temuan || 'Tidak ada temuan.'}</p></div></div></div>}
  </div>;
}
